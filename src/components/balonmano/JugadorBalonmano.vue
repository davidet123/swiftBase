...
<script setup>
  ...
  const { partido, marcador, suspensionesActivasLocal, suspensionesActivasVisitante, estadisticaManual, editarEstadisticas } = storeToRefs(balonmanoStore)
  ...
  const metodoInfoPlayer = () => {
    if(equipo.value === "local") return "infoPlayerHome"
    if(equipo.value === "visitante") return "infoPlayerAway"
  }
